...
public class AutoValueHandler {
    ...
    private Object autoValue(ProceedingJoinPoint joinPoint) throws Throwable {
        List<Object> list = Arrays.asList(joinPoint.getArgs());
        try {
    ...
    private Object activeAutoValue(ProceedingJoinPoint joinPoint, List<Object> list, Boolean isSave) throws Throwable {
        list.forEach(arg -> {
    ...
    private void getAnnotation(Object bean, long updateVersion, Boolean isSave) throws Exception {
        // 利用Java反射机制注入属性
        Class<?> aClass = bean.getClass();
        List<Field> fields = new ArrayList<>(Arrays.asList(aClass.getDeclaredFields()));
        while (!BaseEntityVersion.class.equals(aClass) && !BaseEntity.class.equals(aClass)) {
